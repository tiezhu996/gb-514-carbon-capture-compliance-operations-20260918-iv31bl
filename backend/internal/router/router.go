...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	reviewContextRepository := repository.NewReviewContextRepository(db)
	...
	complianceDecisionService := service.NewComplianceDecisionService(complianceDecisionRepository, securityService, reviewContextRepository)
